...
$result1 = mysqli_query($mysqli, "SELECT * FROM header_subjects ORDER BY id DESC"); 
...
//	$pcat = mysqli_real_escape_string($mysqli, $_POST['pcat']);
	...
	if(empty($pname)) {				
		...
		if(empty($imageName)) {
			$result = mysqli_query($mysqli, "UPDATE  `header_subjects` SET  `product_name` = '$pname' WHERE `id` =$id");
			header("Location: view_class_video.php");			 
		} 
		else{			
			$fl_name=$_SESSION['ex_name_2'];
			$path="upload/".$imageName;
			$result = mysqli_query($mysqli, "UPDATE  `header_subjects` SET  `product_name` = '$pname', `product_img_path`=''  WHERE `id` =$id");
		 	move_uploaded_file($_FILES['image']['tmp_name'],'upload/'.$imageName);
			$result = mysqli_query($mysqli, "UPDATE  `header_subjects` SET  `product_name` =  '$pname',  `product_img_path`='$path'  WHERE `id` =$id");
			header("Location: view_class_video.php");
		}
	}
}
...
<?php
 $id = $_GET['id'];
$result = mysqli_query($mysqli, "SELECT * FROM `header_subjects` LEFT join subject_chapter on header_subjects.chapter_id = subject_chapter.chapterrefid WHERE id = $id");
while($res = mysqli_fetch_array($result))
{
	...
	$product_img_path = $res['product_img_path'];
	$fname = $res['product_img_path'];	
	$_SESSION['ex_name_2']=$ex_name_2 = basename($fname);          // "index.php"	 
} 
?>
<!doctype html>
        ...
        <header class="top-bar">
            <nav class="navbar navbar-default">
                <ul class="nav navbar-nav">
                    ...
                    <li class="page-title">
                        <h2>Edit Products Information</h2>
                    </li>
                </ul>                 
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default panelMove panelFullscreen panelToggle panelClose panelRefresh">
                                ...
                                <div class="panel-body">
                                     <form class="form-horizontal group-border stripped" enctype="multipart/form-data" method="post" action="update_class_video.php">
                                        
                                        <!-- End .form-group  -->
										<div class="form-group">
                                            <label class="col-lg-2 col-md-3  control-label" for="">Chapter Name</label>
                                             <div class="col-lg-10 col-md-9">
											  
                                                  <div class="col-lg-10 col-md-9">
												  <input readonly type="text" name="pcat1" class="form-control" value="<?php echo $chapter_name;?>" id="passwordfield" placeholder="Product Name">
                                                
                                            </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-lg-2 col-md-3  control-label" for="">Topic Name</label>
                                            <div class="col-lg-10 col-md-9">
                                                <input type="text" name="pname" class="form-control" value="<?php echo $product_name;?>" id="passwordfield" placeholder="Product Name">
                                            </div>
                                        </div> 
										
                                        <div class="form-group">
                                            <label class="col-lg-2 col-md-3 control-label" for="">Video Update</label>
                                            <div class="col-lg-10 col-md-9">                                              
                                                     
                                                    <input type="file" name="image" class="form-control" id="fileUpload" >
                                                     <span><?php echo $ex_name_2; ?></span>
