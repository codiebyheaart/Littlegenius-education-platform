...
$id = mysqli_real_escape_string($mysqli, $_POST['id']);	
	...
	if(empty($pname) || empty($pcat)) {				
		...
		if(empty($imageName)) {
			$result = mysqli_query($mysqli, "UPDATE  `product_image` SET  `product_name` =  '$pname', `cat_id`='$pcat'  WHERE `id` =$id");
			header("Location: view_products.php");			 
		} 
		else{			
			$fl_name=$_SESSION['ex_name'];
			$path="upload/".$fl_name;
			$result = mysqli_query($mysqli, "UPDATE  `product_image` SET  `product_name` =  '$pname', `cat_id`='$pcat' , `product_img_path`=''  WHERE `id` =$id");
		 	move_uploaded_file($_FILES['image']['tmp_name'],'upload/'.$fl_name);
			$result = mysqli_query($mysqli, "UPDATE  `product_image` SET  `product_name` =  '$pname', `cat_id`='$pcat' , `product_img_path`='$path'  WHERE `id` =$id");
			header("Location: view_products.php");
		}
	}
}
...
<?php
$id = $_GET['id'];
$result = mysqli_query($mysqli, "SELECT * FROM product_image WHERE id=$id");
while($res = mysqli_fetch_array($result))
{
	...
	$_SESSION['ex_name']=$ex_name = basename($fname);          // "index.php"	 
} 
?>
<!doctype html>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default panelMove panelFullscreen panelToggle panelClose panelRefresh">
                                ...
                                <div class="panel-body">
                                     <form class="form-horizontal group-border stripped" enctype="multipart/form-data" method="post" action="edit_product.php">
                                        
                                        <!-- End .form-group  -->
                                        <div class="form-group">
                                            <label class="col-lg-2 col-md-3  control-label" for="">Product Name</label>
										...
										<div class="form-group">
                                            <label class="col-lg-2 col-md-3  control-label" for="">Product Category</label>
                                             <div class="col-lg-10 col-md-9">
                                                  ...
                                                  <div class="col-lg-10 col-md-9">
												  <input readonly type="text" name="pcat1" class="form-control" value="<?php echo $cat_id;?>" id="passwordfield" placeholder="Product Name">
                                                <select  name="pcat" class="fancy-select form-control fancified" >
												<?php							 
												while($res1 = mysqli_fetch_array($result1)) { 		
													?>
													  <option value="<?php echo $res1['id']; ?>"><?php echo $res1['cat_name']; ?>&nbsp(<?php echo $res1['id']; ?>)</option>
													<?php
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-10 col-md-9">                                              
                                                    ...
                                                    <input type="file"   name="image" class="form-control" name="fileUpload" id="fileUpload" >
